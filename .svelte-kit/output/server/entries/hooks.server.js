...
import { p as private_env, b as public_env } from "../chunks/shared-server.js";
import "../chunks/utils.js";
import "@sveltejs/kit";
import "clsx";
import "@sveltejs/kit/internal";
import "../chunks/query.js";
import { isProduction, isTest, isDevelopment, env, logger, shouldPublishLog, createLogger } from "@better-auth/core/env";
...
import { i as isDynamicBaseURLConfig, w as wildcardMatch, a as getOrigin, b as getHost, c as getProtocol, g as getBaseURL, r as resolveBaseURL } from "../chunks/error-codes.js";
import * as import__better_auth_core_db from "@better-auth/core/db";
import { getAuthTables } from "@better-auth/core/db";
import { filterOutputFields } from "@better-auth/core/utils/db";
import { hkdf } from "@noble/hashes/hkdf.js";
import { sha256 } from "@noble/hashes/sha2.js";
import { decodeProtectedHeader, jwtDecrypt, calculateJwkThumbprint, base64url, jwtVerify, EncryptJWT, SignJWT, generateKeyPair, exportJWK, importJWK } from "jose";
...
import { APIError as APIError$1, toResponse, kAPIErrorHeaderSymbol, createRouter } from "better-call";
...
import { initGetModelName, initGetFieldName, whereOperators } from "@better-auth/core/db/adapter";
...
async function getTrustedOrigins(options, request) {
  ...
  if (isDynamicBaseURLConfig(options.baseURL)) {
    ...
    for (const host of allowedHosts) if (!host.includes("://")) {
      ...
      if (host.includes("localhost") || host.includes("127.0.0.1")) trustedOrigins2.push(`http://${host}`);
...
async function getAdapter(options) {
  return getBaseAdapter(options, async (opts) => {
    const { createKyselyAdapter: createKyselyAdapter2 } = await import("../chunks/index.js");
    ...
    const { kyselyAdapter } = await import("../chunks/index.js");
...
async function generateExportedKeyPair(options) {
  const { alg: alg2, ...cfg } = options?.jwks?.keyPairConfig ?? {
    alg: "EdDSA",
    crv: "Ed25519"
  };
  const { publicKey, privateKey } = await generateKeyPair(alg2, {
    ...cfg,
    extractable: true
  });
  return {
...
async function createJwk(ctx, options) {
  const { publicWebKey, privateWebKey, alg: alg2, cfg } = await generateExportedKeyPair(options);
...
async function signJWT(ctx, config2) {
  const { options } = config2;
  const payload = config2.payload;
  ...
  const alg2 = key.alg ?? options?.jwks?.keyPairConfig?.alg ?? "EdDSA";
  const privateKey = await importJWK(JSON.parse(privateWebKey), alg2);
...
async function verifyJWT(token, options) {
  ...
  try {
    ...
    ctx.context.logger.debug("JWT verification failed", error2);
    return null;
  }
}
    ...
    endpoints: {
      getJwks: createAuthEndpoint(jwksPath, {
        ...
        const keys = keySets.filter((key) => {
          ...
          return key.expiresAt.getTime() + gracePeriod > now2;
        });
...
function resolveAuthSecret() {
  const configuredSecret = nonEmpty(private_env.BETTER_AUTH_SECRET);
  if (configuredSecret) {
...
const siteUrl = nonEmpty(public_env.PUBLIC_SITE_URL) ?? nonEmpty(private_env.SITE_URL) ?? "http://localhost:5173";
